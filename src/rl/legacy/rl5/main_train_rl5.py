# src/rl/main_train_rl5.py
"""
RL-5 training: single DQN agent acting at all five stages.

Process: Picking → Quality Check → Packing → Labelling → Dispatch
...
def _weighted_choice(
    items: List[dict], probs: List[float], rng: random.Random
) -> dict:
    r = rng.random()
    acc = 0.0
...
def _make_resources(
    base_resources: Dict, workers: Tuple[int, int, int, int, int]
) -> Dict:
    r = dict(base_resources)
    r["picking_workers"]       = int(workers[0])
    r["quality_check_workers"] = int(workers[1])
    r["packing_workers"]       = int(workers[2])
    r["labelling_workers"]     = int(workers[3])
    r["dispatch_workers"]      = int(workers[4])
    return r
...
def main() -> None:
    ...
    buffer = ReplayBuffer(capacity=int(rl_cfg["buffer"]["capacity"]))

    mix       = rl_cfg.get("scenario_mix", {})
    scenarios = mix.get("scenarios", []) if bool(mix.get("enabled", True)) else []
    if not scenarios:
        scenarios = [{"name": "default", "prob": 1.0, "workers": [1, 1, 1, 1, 1]}]
    ...
    with open(hist_path, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow([
            ...
            "qc_dec_pts",    "qc_pct_urgent",
            "pack_dec_pts",  "pack_pct_urgent",
            "lab_dec_pts",   "lab_pct_urgent",
    ...
    for ep in range(1, episodes + 1):
        ...
        qc_dec    = int(metrics.get("qc_dec_pts",    0))
        qc_pu     = float(metrics.get("qc_pct_urgent",    0.0))
        pack_dec  = int(metrics.get("pack_dec_pts",  0))
        pack_pu   = float(metrics.get("pack_pct_urgent",  0.0))
        lab_dec   = int(metrics.get("lab_dec_pts",   0))
        lab_pu    = float(metrics.get("lab_pct_urgent",   0.0))
        ...
        w_str    = "-".join(str(x) for x in workers)
        print(
            f"[EP {ep:03d}] scen={scen.get('name', '?')} W={w_str} "
            f"SLA={sla:.4f} U={sla_u:.4f} N={sla_n:.4f} ma5={ma5:.4f} | "
            f"eps={eps:.3f} loss={loss_str} buf={len(buffer)} "
            f"upd={updates_done} grad={grad_step_total} | "
            f"dec={total_dec} %U={p_urgent:.2f} "
            f"[pick:{pick_dec}/{pick_pu:.2f} qc:{qc_dec}/{qc_pu:.2f} "
            f"pack:{pack_dec}/{pack_pu:.2f} lab:{lab_dec}/{lab_pu:.2f} "
            f"disp:{disp_dec}/{disp_pu:.2f}] | "
            f"t={runtime:.1f}s"
        )
        ...
        with open(hist_path, "a", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow([
                ep, scen.get("name", "s"), w_str, start_idx,
                sla, sla_u, sla_n, ma5,
                eps,
